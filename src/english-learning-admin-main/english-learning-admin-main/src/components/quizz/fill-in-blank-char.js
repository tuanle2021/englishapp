...
export const FillInBlankChar = ({ ...props }) => {


    const title = "Fill in blank char";
                            ...
                            <>
                                <Grid sx={{ mt: 3 }}>
                                    ...
                                    <Grid item md={12} sx={{ mt: 3 }} >
                                        <Grid container>
                                            {formik.values.quizz_detail.present && formik.values.quizz_detail.present.length > 0 ? (
                                                formik.values.quizz_detail.present.map((phase, index) => (
                                                    <Grid key={index} xs={4} sx={{ mt: 3 }}  >

                                                        <TextField
                                                            name={`quizz_detail.present.${index}`}
                                                            sx={{ mr: 3 }}
                                                            label={`present-${index}`}
                                                            onChange={formik.handleChange}
                                                            value={formik.values.quizz_detail.present[index]} />
